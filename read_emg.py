...
flags.DEFINE_list('silent_data_directories', ['../silent_parallel_data'], 'silent data locations')
flags.DEFINE_list('voiced_data_directories', ['../voiced_parallel_data','../nonparallel_data'], 'voiced data locations')
...
class EMGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        if directory_info.silent: 
            ...
            result['paired_raw_emg'] = torch.from_numpy(voiced_raw_emg).pin_memory()

            audio_file = f'{voiced_directory.directory}/{voiced_idx}_audio_clean.flac'

        result['phonemes'] = torch.from_numpy(phonemes).pin_memory() 
    ...
    def collate_raw(batch):
        ...
        for ex in batch:
            if ex['silent']:
                ...
                paired_raw_emg.append(ex['paired_raw_emg'])
            else:
                ...
                paired_raw_emg.append(ex['raw_emg'])
        ...
        paired_lengths = [ex.shape[0] // 8 for ex in paired_raw_emg]
