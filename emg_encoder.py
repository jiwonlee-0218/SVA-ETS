...
def init_emg_encoder( model_config, device: torch.device = None, ) -> EMGEncoder:  
    
    emg_encoder_config = model_config                
    num_ins: int = 8
    
    
    num_outs: int = SPEECH_UNITS_FEAT_SIZE      
    num_aux_outs: int = len(PHONEME_INVENTORY)  
...
def load_emg_encoder( cfg: omegaconf.DictConfig, device: torch.device, emg_encoder_checkpoint_path: Path, ) -> EMGEncoder:
